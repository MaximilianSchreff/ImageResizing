...
public class ContentAwareImageResizing {
    ...
    public int[] leastContrastImageVPath() {
        ShortestPathsTopological shortestPaths = new ShortestPathsTopological(makeVGraph(), sx * sy);
        Stack<Integer> pathStack = shortestPaths.pathTo(sx * sy + 1);
        int[] path = new int[sy];

        //pop the start node
        pathStack.pop();
        for (int i = 0; i < sy; i++) {
            path[i] = this.nodeToCoordinate(pathStack.pop()).x;
        }
        return path;
    }
